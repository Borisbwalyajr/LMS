        ...
        <form id="filterForm" class="myForm" method="post">
            <div class="form-row" align="left">
                ...
                <div class="form-group col-md-3">
                    ...
                    <?php
                    $loan_date_options = '';
                    $query = "SELECT DISTINCT loan_date FROM loan_applications ORDER BY loan_date ASC";
                    $stmt = $con->query($query);
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        $loan_date_options .= '<option value="' . $row["loan_date"] . '">' . $row["loan_date"] . '</option>';
                    }
                    ?>
                    <select name="loan_date" id="loan_date" class="custom-select">
                        <option value="">--Select Loan Date--</option>
